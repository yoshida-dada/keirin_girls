...
def predict_race_dict(kaisai_code: str, day_code: str, race_no: int,
    ...
    def _mins_to_dl(dl: str | None) -> float | None:
        if not dl or ":" not in dl:
            return None
        from datetime import datetime as _d2, timezone as _t2, timedelta as _td2
        _n = _d2.now(_t2(_td2(hours=9)))
        try:
            _h, _m = (int(x) for x in dl.split(":"))
        except ValueError:
            return None
    ...
    if odds and strengths and (_mtd is None or _mtd <= 180):
        try:
            ...
            _ex_odds = fetch_exacta_odds(kaisai_code, day_code, race_no)
            if _ex_odds:
                exacta_block = select_exacta(strengths, probs, _ex_odds, n_points=3, pool=4)
        except Exception:
            exacta_block = None
