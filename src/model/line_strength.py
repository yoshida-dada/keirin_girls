...
def build_lines(lines: list[list[int]], strengths: dict[int, float],
    ...
    return {
        ...
        # 別ラインへ散らすかの判断材料。
        # **2026-08-18 に本表示の分布を分岐混合へ切り替えて、過小評価は解消した。**
        # 以前は紐補正で 35.3%（実測55.6%に対し −20.3pt）だったが、混合は 56.8%（+1.2pt）。
        # ただし混合が作れないレース（ガールズ・並び予想なし）は紐補正のままなので、
        # その場合は依然として20pt前後低く出る。どちらで出しているかは
        # レースの prob_source で判別できる。
        "settle_prob": settle,
        # 実測基準（男子25,155R）。表示側でモデル値と並べて読ませる。
        "settle_baseline": 0.562,
        "settle_bias_note": "分岐混合で出している場合は実測とほぼ一致する"
                            "（walk-forward n=12,420: 予測56.8% vs 実測55.6%）。"
                            "並び予想が無く紐補正になる場合のみ、実測より約20pt低く出る。",
